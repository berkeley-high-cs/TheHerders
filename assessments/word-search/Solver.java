...
public class Solver {
        ...
        if (puzzle[x][y].equals(word.substring(0, 1))) { //check if its equal to the first charcter in word
          ...
          for (int i = 0; i < 8; i++) { //if it is, loop 8 times checking around that place 
            ...
            if (checkAround(x, y, i, puzzle, word)) { // if it finds the next character, 
              ...
              for (int z = 1; z < word.length(); z++) { // it loops through the word count minus the words weve checked

                if (checkAround(x, y, z, i, puzzle, word)) { //and goes as far as it can
  ...
  public boolean checkAround(
    ...
    return (
      inBounds(x + xAdds[i] * z, y + yAdds[i] * z, puzzle) &&
      word.substring(z, z+1).equals(puzzle[x + xAdds[i]][y + yAdds[i]])
    );
  }
}
